...
export class EntregaComponent implements OnInit {
  ...
  selection = new SelectionModel<EntregaItem>(true, []);
  ...
  entregaLiquidacion: EntregaLiquidacion;


  dataSourceEntregaItem: MatTableDataSource<EntregaItem>;
  dataSourceEntregaLiq: MatTableDataSource<EntregaLiquidacion>;
  ...
  constructor(private clientesService: ClientesService, private dialogo: MatDialog,
    ...
    editItem(it:EntregaItem){

      var dialogRef;
  
      dialogRef=this.dialog.open(DialogEditItemEntrega,{
        data:it,
      })
    ...
    applyFilterD(event: Event) {
      ...
      this.dataSourceEntregaItem.filter = filterValue.trim().toLowerCase();
  
      if (this.dataSourceEntregaItem.paginator) {
        this.dataSourceEntregaItem.paginator.firstPage();
      }
    }
  ...
  /*     var year= this.fecha.getFullYear();
    ...
    salaChange(){
      this.logisticaService.getSalaByName(this.sala).subscribe((res4:Campus)=>{
        if(res4){
          this.entregaLiquidacion.campus=this.sala;
          this.entregaLiquidacion.campus_dir=res4.address;
          this.entregaLiquidacion.empresa=res4.company;
          this.ord_suffix=res4.supply_ord_suffix;
          this.entregaLiquidacion.numero=res4.supply_ord_suffix;
    ...
    addEntregaItem(){
      ...
      this.entregaItem=new  EntregaItem(this.sala,this.fechaStr,'','','','','','','','PENDIENTE',0,this.user.user_id,0);
      this.selection.clear();
  
      dialogRef=this.dialog.open(DialogNewItemEntrega,{
        data:{item:this.entregaItem,
              date:this.fecha},
      })
    ...
    addEntregaLiquidacion(){
      ...
      this.entregaLiquidacion.importe='';

      this.entregaLiquidacion.personal=this.user.first_name+' '+this.user.paternal_surname+' '+this.user.maternal_surname;
      this.entregaLiquidacion.user_id=this.user.user_id;
      ...
      this.logisticaService.getLastEntregaLiquidacionNum(this.entregaLiquidacion.numero,this.entregaLiquidacion.campus,this.entregaLiquidacion.empresa).subscribe(resp=>{
        if(resp['numero']){
          ...
          this.entregaLiquidacion.numero=this.ord_suffix+'-'+numStr;
        }
        else{
          this.entregaLiquidacion.numero=this.ord_suffix+'-0001';
        }


        this.entregaLiquidacion.importe=this.displayImporte;
              ...
              this.logisticaService.updateEntregaItem(i).subscribe(resr=>{
                if(ind==this.selection.selected.length-1&&resr){
                  ...
                  console.log(this.entregaLiquidacion);
                      ...
                      this.generatePDF(this.entregaLiquidacion);
    ...
    createTxtFile(fl:EntregaLiquidacion){
    ...
    generatePDF(fondliq:EntregaLiquidacion){
      var empresaCompleto='';
  ...
  /*       this.doc.roundedRect(0, 100, 210, 10, 0, 0, 'S'); */
    ...
    ngOnInit() {
      ...
      this.entregaLiquidacion=new EntregaLiquidacion('','','','','','','',this.user.user_id,'');
              ...
              if(ar){
                ...
                this.logisticaService.getCampusById(this.colab.campus_id).subscribe((camp:Campus)=>{
                  if(camp){
                    ...
                    this.entregaLiquidacion.campus=this.sala;
                    this.entregaLiquidacion.campus_dir=this.user_campus.address;
                    this.entregaLiquidacion.empresa=this.user_campus.company;
                    this.ord_suffix=this.user_campus.supply_ord_suffix;
                    this.entregaLiquidacion.numero=this.user_campus.supply_ord_suffix;
                        ...
                        if(resi){
                          ...
                          this.logisticaService.getEntregaLiquidacionesByCampus(this.sala).subscribe((liqs:EntregaLiquidacion[])=>{
                            ...
                            this.logisticaService.getEntregaItems(this.sala,'PENDIENTE',this.user.user_id).subscribe((res:EntregaItem[])=>{
                              ...
                              this.dataSourceEntregaItem.sort = this.sort.toArray()[1];
                            })
...
/*   @ViewChild(MatPaginator, { static: false }) paginator2= new QueryList<MatPaginator>();
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogNewItemEntrega>,
...
@Component({
  selector: 'dialog-editItemEntrega',
  templateUrl: 'dialog-editItemEntrega.html',
  styleUrls: ['./entrega.component.css']
})
export class DialogEditItemEntrega implements OnInit {
...
/*   @ViewChild(MatPaginator, { static: false }) paginator2= new QueryList<MatPaginator>();
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogEditItemEntrega>,
    @Inject(MAT_DIALOG_DATA) public data:EntregaItem,
    private fb: FormBuilder,
    private usersService: UsersService,
    private logisticaService: LogisticaService,
    private toastr: ToastrService,
...
@Component({
  selector: 'dialog-confirmEntrega',
  templateUrl: 'dialog-confirmEntrega.html',
  styleUrls: ['./entrega.component.css']
})
export class DialogConfirmEntrega implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogConfirmEntrega>,
    @Inject(MAT_DIALOG_DATA) public data,
    private fb: FormBuilder,
    private usersService: UsersService,
    private logisticaService: LogisticaService,
