...
// Definir la parte adicional para el LEFT JOIN y WHERE
$left_join_clause = "LEFT JOIN oscorp_supply.orders_validations c ON a.id = c.order_id 
    ...
    if ($destino == 'NINGUNO') {
        ...
        if ($destino == 'TODOS') {
            $sentencia = $bd->prepare("SELECT a.*, COALESCE(CONCAT(b.serie, '-', b.numero), 'SN') AS comprobante FROM (
                ...
                ordenes.estado, ordenes.empresa, ordenes.moneda, ordenes.area, ordenes.destino_dir, ordenes.tipo_pago, ordenes.num_cuenta,
                ordenes.retencion, ordenes.retencion_percent, ordenes.percepcion, ordenes.receipt, ordenes.txt, ordenes.section, ordenes.status, ordenes.observacion 
                FROM oscorp_supply.ordenes WHERE ordenes.section='OFICINA' ) a 
                LEFT JOIN oscorp_supply.fondoitems b ON a.id = b.orden_id 
                ".$left_join_clause."
                ORDER BY a.id DESC;");
        } else {
            $sentencia = $bd->prepare("SELECT a.*, COALESCE(CONCAT(b.serie, '-', b.numero), 'SN') AS comprobante FROM (
                ...
                ordenes.estado, ordenes.empresa, ordenes.moneda, ordenes.area, ordenes.destino_dir, ordenes.tipo_pago, ordenes.num_cuenta,
                ordenes.retencion, ordenes.retencion_percent, ordenes.percepcion, ordenes.receipt, ordenes.txt, ordenes.section, ordenes.status, ordenes.observacion 
                FROM oscorp_supply.ordenes WHERE ordenes.section='OFICINA' AND ordenes.destino='".$destino."' ) a 
                LEFT JOIN oscorp_supply.fondoitems b ON a.id = b.orden_id 
                ".$left_join_clause."
                ORDER BY a.id DESC;");
        }
    ...
    if ($user_role == 'SUPER ADMINISTRADOR') {
        if ($destino == 'NINGUNO') {
            ...
            if ($destino == 'TODOS') {
                $sentencia = $bd->prepare("SELECT a.*, COALESCE(CONCAT(b.serie, '-', b.numero), 'SN') AS comprobante FROM (
                    SELECT ordenes.id, ordenes.req_id, ordenes.numero, ordenes.ruc, ordenes.razon_social, ordenes.direccion, 
                    ordenes.subtotal, ordenes.igv, ordenes.total, ordenes.rebajado, ordenes.total_inicial, ordenes.fecha, 
                    ordenes.destino, ordenes.tipo, ordenes.estado, ordenes.empresa, ordenes.moneda, ordenes.area, ordenes.destino_dir, 
                    ordenes.tipo_pago, ordenes.num_cuenta, ordenes.retencion, ordenes.retencion_percent, ordenes.percepcion, 
                    ordenes.receipt, ordenes.txt, ordenes.section, ordenes.status, ordenes.observacion 
                    FROM oscorp_supply.ordenes WHERE ordenes.section='OFICINA' ) a 
                    LEFT JOIN oscorp_supply.fondoitems b ON a.id = b.orden_id 
                    ".$left_join_clause."
                    ORDER BY a.id DESC;");
